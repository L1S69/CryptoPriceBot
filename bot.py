...
languages = keyboards.create_keyboard(2,
...
def send_settings(message):
  ...
  if len(message.text.split()) < 2:
    ...
    to_curr = args[2]
    try:
      conversion_result = cgapimanager.perform_currency_conversion(
...
def set_fiat_currency(message):
  ...
  if message.text in config.supported_fiat_currencies:
    ...
    bot.send_message(message.chat.id,
                     f"{lang[9][0]}{fiat_currency}{lang[9][1]}",
                     reply_markup=cryptos)
  else:
...
def set_language(message):
  if config.supported_languages[message.text]:
    dbmanager.set_data(message.chat.id, "language",
                       config.supported_languages[message.text])
    # Notify user that his default fiat currency was changed
    bot.send_message(
        message.chat.id,
        tlmanager.languages[config.supported_languages[message.text]][7],
        reply_markup=cryptos)
...
def get_text_messages(message):
  ...
  currency = dbmanager.get_data(message.chat.id, "currency",
                                "usd")  # Get user's default fiat currency
  lang = tlmanager.get_language(message.chat.id)
  data = cgapimanager.get_crypto_price(text, currency, True)
  ...
  graph = graphmanager.generate_price_change_graph(timestamps, prices,
                                                   percent_change > 0,
